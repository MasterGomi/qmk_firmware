...
void lambda_plus_tap_dance(qk_tap_dance_state_t *state, void *user_data){
    if (state->count == 1){
        SEND_STRING("=> ");
    }
    else {
...
qk_tap_dance_action_t tap_dance_actions[] = {
    [LAMBDA_PLUS] = ACTION_TAP_DANCE_FN(lambda_plus_tap_dance)
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT(
                    C(S(KC_ESC)),      C(S(KC_S)),    LWIN(S(KC_S)),             LWIN(KC_L),
                    KC_NO,      KC_NO,    KC_NO,             KC_NO,
                    KC_F12,      KC_NO,    C(S(KC_R)),             ALT_TAB,
        KC_MEDIA_PLAY_PAUSE,    TD(LAMBDA_PLUS),     C(S(KC_F)),    KC_F5,   SHIFT_ALT_TAB
    ),
   /*
    [1] = LAYOUT(
